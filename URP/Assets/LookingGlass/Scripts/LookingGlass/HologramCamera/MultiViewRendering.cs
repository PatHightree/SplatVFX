...
namespace LookingGlass {
    internal static class MultiViewRendering {
        ...
        internal static void Clear(RenderTexture renderTarget, CameraClearFlags clearFlags, Color color) {
            ...
            color = clearColor ? color : Color.clear;
        ...
        internal static void RenderQuilt(HologramCamera hologramCamera, bool ignorePostProcessing, Action<int> onViewRender) {
            ...
            if (onlyShowViewIndex > -1) {
                ...
                for (int i = 0; i < data.renderSettings.numViews; i++) {
                    if (viewInterpolation.IsInterpolatedView(i, data.renderSettings.numViews))
                        continue;
        ...
        public static void InterpolateViewsOnQuilt(HologramCamera hologramCamera, RenderTexture quilt, RenderTexture quiltRTDepth) {
            ...
            int viewInterpolation = optimization.ViewInterpolation.GetViewIndexDivisor(renderSettings.numViews);
...
// #endif
            ...
            // Only dim the edge views for 32 and 65 inch
            switch (cal.GetDeviceType()) {
                case DeviceType._32in:
                case DeviceType._65in:
                    lightfieldMaterial.SetInt(lightfieldProperties.filterEdge, 1);
                    break;
                default:
                    lightfieldMaterial.SetInt(lightfieldProperties.filterEdge, 0);
                    break;
            }
        }
    }

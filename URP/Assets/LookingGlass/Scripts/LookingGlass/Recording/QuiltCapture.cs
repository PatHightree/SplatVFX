    ...
    public sealed class QuiltCapture : MonoBehaviour {
        ...
        public QuiltRecordingSettings CustomRecordingSettings {
            get {
                ...
                if (result.cameraOverrideSettings.renderSettings.aspect <= 0)
                    result.cameraOverrideSettings.renderSettings.aspect = HologramCamera.Aspect;
                return result;
            }
            set {
                if (state != QuiltCaptureState.NotRecording)
        ...
        public QuiltRecordingSettings RecordingSettings {
            get {
                ...
                if (settings.cameraOverrideSettings.renderSettings.aspect <= 0)
                    settings.cameraOverrideSettings.renderSettings.aspect = HologramCamera.Aspect;
        ...
        public QuiltCaptureOverrideSettings CustomScreenshotSettings {
            get {
                ...
                if (result.renderSettings.aspect <= 0)
                    result.renderSettings.aspect = HologramCamera.Aspect;
                return result;
            }
            set {
                if (state != QuiltCaptureState.NotRecording)
...
#region File Naming
        ...
        private string CalculateAutoCorrectPath(QuiltCaptureMode captureMode) {
            HologramRenderSettings renderSettings = OverrideSettings.renderSettings;
            Assert.IsTrue(renderSettings.aspect > 0);
            return CalculateAutoCorrectPath(captureMode, renderSettings.viewColumns, renderSettings.viewRows, renderSettings.aspect, TakeNumber);
        }
        public string CalculateAutoCorrectPath(QuiltCaptureMode captureMode, int viewColumns, int viewRows, float finalAspect, int takeNumber) {
            if (finalAspect <= 0)
                throw new ArgumentOutOfRangeException(nameof(finalAspect), finalAspect,
                    "The aspect must be greater than zero. See " + nameof(HologramCamera) + "." + nameof(HologramCamera.Aspect) + ".");
...
#region Recording Methods
        ...
        private async Task StartRecordingFramesInternal(int startFrame, int endFrame, OutputFilePath outputFilePath) {
            ...
            StartCoroutine(FrameIntervalRecordingCoroutine(startFrame, endFrame, tcs));
            try {
                await tcs.Task;
            } catch (Exception e) {
        ...
        private IEnumerator FrameIntervalRecordingCoroutine(int startFrame, int endFrame, TaskCompletionSource<bool> tcs) {
